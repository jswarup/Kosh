//-- stash.rs -------------------------------------------------------------------------------------------------------------------------
use	std::{ mem::replace, ptr::copy_nonoverlapping };
use	crate::silo::{ Arr, Buff, IAccess, IArr, Stk, U32, U8 };
use	crate::silo::cast::ICastExt;
use	crate::stalks::Atm;
use	std::sync::atomic::Ordering;

//---------------------------------------------------------------------------------------------------------------------------------

pub struct Stash< T>
{
    _Buff: Buff< T>,
    _Sz: Atm< U32>,
}

//---------------------------------------------------------------------------------------------------------------------------------

impl< T> Stash< T>
{

    //-----------------------------------------------------------------------------------------------------------------------------

    pub fn	New() -> Self
    {
        Self {
            _Buff: Buff::New(),
            _Sz: Atm::New( U32( 0)),
        }
    }

    //-----------------------------------------------------------------------------------------------------------------------------

    pub fn	WithCapacity< Sz: Into< U32>>( capacity: Sz) -> Self
    where
        T: Default,
    {
        let  	cap = capacity.into();
        Self {
            _Buff: Buff::Create( cap, |_| T::default()),
            _Sz: Atm::New( U32( 0)),
        }
    }

    //-----------------------------------------------------------------------------------------------------------------------------

    pub fn	WithCapacityVal< Sz: Into< U32>>( capacity: Sz, fillVal: T) -> Self
    where
        T: Copy,
    {
        let  	cap = capacity.into();
        Self {
            _Buff: Buff::Create( cap, |_| fillVal),
            _Sz: Atm::New( U32( 0)),
        }
    }

    //-----------------------------------------------------------------------------------------------------------------------------

    pub fn	FromBuff( buff: Buff<T>, szStk: U32) -> Self
    {
        Self {
            _Buff: buff,
            _Sz: Atm::New( szStk),
        }
    }

    //-----------------------------------------------------------------------------------------------------------------------------

    pub fn	Create< Sz: Into< U32>, SzStk: Into< U32>, Dispenser>(
        sz: Sz,
        szStk: SzStk,
        dispenser: Dispenser,
    ) -> Self
    where
        Dispenser: Fn( U32) -> T,
    {
        Self {
            _Buff: Buff::Create( sz, dispenser),
            _Sz: Atm::New( szStk.into()),
        }
    }

    //-----------------------------------------------------------------------------------------------------------------------------

    pub fn	Size( &self) -> U32
    {
        self._Sz.Load( Ordering::Acquire)
    }

    //-----------------------------------------------------------------------------------------------------------------------------

    pub fn	Capacity( &self) -> U32
    {
        self._Buff.Size()
    }

    //-----------------------------------------------------------------------------------------------------------------------------

    pub fn	Clear( &self)
    {
        self._Sz.Store( U32( 0), Ordering::Release);
    }

    //-----------------------------------------------------------------------------------------------------------------------------

    pub fn	Stk( &self) -> Stk< '_, '_, T>
    {
        Stk::Create( &self._Sz, self._Buff.Arr())
    }

    //-----------------------------------------------------------------------------------------------------------------------------

    pub fn	BuffOut( &mut self) -> Buff< T>
    {
        replace( &mut self._Buff, Buff::New())
    }

    //-----------------------------------------------------------------------------------------------------------------------------

    pub fn	IntoBuff( mut self) -> Buff< T>
    {
        let  	sz = self.Size();
        self._Buff.Truncate( sz);
        self.BuffOut()
    }

    //-----------------------------------------------------------------------------------------------------------------------------

    pub fn	ToBuff( &self) -> Buff< T>
    where
        T: Clone,
    {
        let  	sz = self.Size().AsUsize();
        let  	slice = &(*self._Buff.Arr())[..sz];
        Buff::from( slice)
    }

    //-----------------------------------------------------------------------------------------------------------------------------

    pub fn	Pop( &self, val: &mut T) -> bool
    {
        self.Stk().Pop( val)
    }

    //-----------------------------------------------------------------------------------------------------------------------------

    pub fn	TopMut( &self) -> Option< &mut T>
    {
        let  	sz = self.Size();
        if sz > U32( 0) {
            let  	arrStk = self.Stk().Arr();
            Some( arrStk.MutAt( sz - U32( 1)))
        } else {
            None
        }
    }

    //-----------------------------------------------------------------------------------------------------------------------------

}

//---------------------------------------------------------------------------------------------------------------------------------

impl< T: Copy> Stash< T>
{
    //-----------------------------------------------------------------------------------------------------------------------------

    fn	DoGrow( &mut self, fillVal: T)
    {
        let  	newSz = if self._Buff.Size() == U32( 0) {
            U32( 1)
        } else {
            self._Buff.Size() * U32( 2)
        };
        self._Buff.Resize( newSz, |_| fillVal);
    }

    //-----------------------------------------------------------------------------------------------------------------------------

    pub fn	Push( &mut self, val:  T)
    {
        while !self.Stk().Push( val) {
            if self.Size() == self._Buff.Size() {
                self.DoGrow( val);
            }
        }
    }

    //-----------------------------------------------------------------------------------------------------------------------------

    pub fn	Append( &mut self, arr: Arr< '_, T>)
    {
        let  	n = arr.Size();
        if n == U32( 0) {
            return;
        }
        let  	neededSz = self.Size() + n;
        if neededSz > self._Buff.Size() {
            let  	growTo = neededSz.max( self._Buff.Size() * U32( 2));
            self._Buff.Resize( growTo, |_| arr[0]);
        }
        let  	startSz = self.Size();
        unsafe {
            copy_nonoverlapping(
                arr.Ptr(),
                self._Buff._Ptr.cast::< T>().as_ptr().add( startSz.AsUsize()),
                n.AsUsize(),
            );
        }
        self._Sz.Set( startSz + n);
    }
}

//---------------------------------------------------------------------------------------------------------------------------------

impl< T: Default> Stash< T>
{
    //-----------------------------------------------------------------------------------------------------------------------------

    pub fn	PushX( &mut self, val: &mut T)
    {
        while !self.Stk().PushX( val) {
            if self.Size() == self._Buff.Size() {
                let  	newSz = if self._Buff.Size() == U32( 0) {
                    U32( 1)
                } else {
                    self._Buff.Size() * U32( 2)
                };
                self._Buff.Resize( newSz, |_| T::default());
            }
        }
    }

    //-----------------------------------------------------------------------------------------------------------------------------

    pub fn	PushVal( &mut self, mut val: T)
    {
        self.PushX( &mut val);
    }

    //-----------------------------------------------------------------------------------------------------------------------------
    
    pub fn	PopToSize( &self, targetSz: U32)
    {
        let  	mut temp = T::default();
        while self.Size() > targetSz {
            self.Pop( &mut temp);
        }
    }
}

//---------------------------------------------------------------------------------------------------------------------------------

impl< T> Stash< T>
where
    T: From< usize> + Clone,
{
    //-----------------------------------------------------------------------------------------------------------------------------
    
    pub fn	DoIndexSetup( &self)
    {
        let  	arr = self._Buff.Arr();
        arr.DoIndexSetup();
        self._Sz.Store( arr.Size(), Ordering::Release);
    }

    //-----------------------------------------------------------------------------------------------------------------------------
}

//---------------------------------------------------------------------------------------------------------------------------------

impl Stash< U8>
{
    pub fn	ReadFrom( &mut self, reader: &mut impl std::io::Read)
    {
        let  mut temp = [0u8; 4096];
        loop {
            match reader.read( &mut temp) {
                Ok( 0) => break,
                Ok( n) => {
                    let  slice = (&temp[..n]).Cast::<&[U8]>();
                    self.Append( slice.into());
                }
                Err( _) => break,
            }
        }
    }
}


//---------------------------------------------------------------------------------------------------------------------------------

#[macro_export]
macro_rules! Stash {
    (@__ $acc:ident, $exp:expr; for $item:pat in $iter:expr; if $cond:expr) => (
        for $item in $iter {
            if $cond {
                $acc.Push($exp);
            }
        }
    );

    (@__ $acc:ident, $exp:expr; for $item:pat in $iter:expr) => (
        for $item in $iter {
            $acc.Push($exp);
        }
    );

    (@__ $acc:ident, $exp:expr; for $item:pat in $iter:expr; if $cond:expr; $($tail:tt)+) => (
        for $item in $iter {
            if $cond {
                $crate::Stash![@__ $acc, $exp; $($tail)+];
            }
        }
    );

    (@__ $acc:ident, $exp:expr; for $item:pat in $iter:expr; $($tail:tt)+) => (
        for $item in $iter {
            $crate::Stash![@__ $acc, $exp; $($tail)+];
        }
    );

    ($exp:expr; $($tail:tt)+) => ({
        let  	mut ret = $crate::silo::Stash::New();
        $crate::Stash![@__ ret, $exp; $($tail)+];
        ret
    });
}

//---------------------------------------------------------------------------------------------------------------------------------
