    ...
    fn	Match< 'p, 's, R: Read>( &self, parser: &mut Parser<'p, 's, R>) -> bool
    {
        ...
        match self.ChildOp() {
            ...
            ChildOp::Less => {
                ...
                let  	leftChild = self._At( U32( 0));
                let  	rightChild = self._At( U32( 1));
                
                if leftChild.Match( forge.GetParser()) {
                    if rightChild.Match( forge.GetParser()) {
                        let  	endMark = forge.GetParser().Stream().Marker();
                        let  	digest = Digest { _Start: startMark, _End: endMark };
                        if let Some( p) = forge.Parent() {
                ...
                forge.GetParser().Stream().RollTo( startMark);
                false
            }
            ChildOp::Bor => {
                let  	mut forge = BinOpForge {
