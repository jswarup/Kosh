...
fn	TestParserBasic()
{
...

    {
        ...
        let matched = 'h'.Match(&mut forge);
        let mut m = forge.Mark();
        assert!( matched);
        let mut forge = ParseForge::New(&mut parser, m);
        let matched = 'e'.Match(&mut forge);
        m = forge.Mark();
        assert!( matched);

        // Test &str grammar
        let mut forge = ParseForge::New(&mut parser, m);
        let matched = "llo ".Match(&mut forge);
        m = forge.Mark();
        assert!( matched);

        let mut forge = ParseForge::New(&mut parser, m);
        let matched = cs.Match(&mut forge);
        m = forge.Mark();
        assert!( matched);

        // Test failing match (should rollback)
        let mut forge = ParseForge::New(&mut parser, m);
        let matched = "fail".Match(&mut forge);assert!( !matched);
    }
}
...
fn TestPostBoxet() 
{
    ...
    let mut parser = Parser::New(&mut stream);
    let mut forge = ParseForge::New(&mut parser, U32(0));
    let matched = tree.Match(&mut forge);assert!(matched);
}
...
fn TestRgx2() 
{
    ...
    let matched = identRgx.Match(&mut forge);
...
fn TestUIntShard()
{
    ...
    let  	mut parser1 = Parser::New( &mut stream1);
    let mut forge = ParseForge::New(&mut parser1, U32(0));
    let matched = tree.Match(&mut forge);
    let m1 = forge.Mark();
    assert!(matched);
