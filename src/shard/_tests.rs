...
fn	TestParserBasic()
{
    ...
    let  	mut m = U32( 0);
    {
        ...
        let  	matched = {
            let  	g = &'h';
            let res = g.Parse( &mut parser, m, None);
...
fn	TestPostBoxet() 
{
    ...
    let  	mut parser = Parser::New( &mut stream);
    let res = tree.Parse( &mut parser, U32(0, None));
...
fn TestRgx2() 
{
    ...
    let res1 = identRgx.Parse( &mut parser1, U32(0, None));
    ...
    let res2 = identRgx.Parse( &mut parser2, U32(0, None));
...
fn TestUIntShard()
{
    ...
    let  	mut parser1 = Parser::New( &mut stream1);
    let res1 = tree.Parse( &mut parser1, U32(0, None));
    let matched1 = res1.is_some();
    ...
    let  	mut parser2 = Parser::New( &mut stream2);
    let res2 = tree.Parse( &mut parser2, U32(0, None));
    let matched2 = res2.is_some();
    ...
    let res3 = tree.Parse( &mut parser3, U32(0, None));
