...
//! Explorer + document Notebook tab strip.
...
use	crate::frieze::desktop::{
    build_menu_bar, ID_CLOSE, ID_OPEN, ID_THEME_CYBERPUNK, ID_THEME_DARK, ID_THEME_LIGHT, ID_THEME_NORD,
};
...
fn	open_tab_for_path( notebook: &Notebook, state: &SharedState, path: &Path)
{
    ...
    let  	tab = OpenTab::New( path.to_path_buf());
    let  	label = tab._Kind.TabLabel( &tab._Name);

    match tab._Kind {
        TabKind::Pts | TabKind::Obj => {
    ...
    state.borrow_mut()._OpenTabs.push( tab);
}
