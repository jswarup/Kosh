...
9. **Naming & Casing Conventions**:
   ...
   - **Struct Data Members (Fields)**: Preceded by an underscore `_` followed by `PascalCase` (e.g. `_Data`, `_Size`, `_Names`, `_PastVal`, `_CurrentVal`, `_FutureVal`, `_X`). Field type definitions must be aligned into consistent vertical columns.
