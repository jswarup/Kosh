...
| **swarm** | WebGPU host-side compute operations via the `IGpuOp` trait on `wgpu::Device`. Provides device initialization, storage buffer creation, and staging buffer readback. | [gpusop.rs](src/swarm/gpusop.rs) |
