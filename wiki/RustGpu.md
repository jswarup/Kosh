...
Instead of writing shader logic in WebGPU Shading Language (WGSL), Kosh supports compiling a secondary crate—[gcomp](file:///c:/Work/Taregna/Kosh/src/gcomp)—directly to Vulkan-compliant SPIR-V bytecode. The host code dynamically invokes the compiler during test execution, reads the generated bytecode, and submits it to `wgpu` pipelines.

```mermaid
graph TD
    A[Host Rust Code] -->|SpirvBuilder| B[Rust-GPU Compiler]
    B -->|Compiles| C[gcomp crate]
...
## 1. Shader Crate Structure (`gcomp`)

The GPU code resides in the [gcomp](file:///c:/Work/Taregna/Kosh/src/gcomp/) workspace crate:

### Cargo Configuration ([Cargo.toml](file:///c:/Work/Taregna/Kosh/src/gcomp/Cargo.toml))
...
### Shader Logic ([lib.rs](file:///c:/Work/Taregna/Kosh/src/gcomp/src/lib.rs))
